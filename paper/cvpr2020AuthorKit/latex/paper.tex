...
free-run stability and accuracy (test set). Lower NRMSE / higher VAF is better;
...
\begin{table}[t]
\centering
\caption{Phase 2 -- full recurrent architectural sweep (hidden size $H$
$\times$ number of layers $L$; mean over $3$ seeds, std in parentheses).
The best free-run VAF of the whole sweep is in bold. Two-layer variants
are never better than their single-layer counterparts.}
\label{tab:phase2full}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llccccc}
\toprule
\textbf{Cell} & \textbf{$H$} & \textbf{$L$} & \textbf{Params} &
...
Tables~\ref{tab:phase1}--\ref{tab:kalman} (summary in
...
pathology, not insufficient capacity.
